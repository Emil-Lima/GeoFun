...
const SelectCountry = ({ profile, onHomeClick }) => {
  ...
  const getUserChoice = (country) => {
    ...
    const listOfGoodAnswers = getGoodAnswers(
      profile.name,
      userChoice,
      mysteryCountry
    );
    const listOfBadAnswers = getBadAnswers(
      profile.name,
      userChoice,
      mysteryCountry
    );
    ...
    setBadAnswer(
      listOfBadAnswers[Math.floor(Math.random() * listOfBadAnswers.length)]
    );
    setGoodAnswer(
      listOfGoodAnswers[Math.floor(Math.random() * listOfGoodAnswers.length)]
    );
  ...
  return (
    <div>
      ...
      <PlayGame>
        ...
        <QuestionContainer>
          {mysteryCountry === null ? null : (
            <p>Find {mysteryCountry} on the map!</p>
          )}
        </QuestionContainer>
      </PlayGame>
        ...
        {europeData.features.map((country, index) => {
          ...
          return (
            <GeoJSON
              ...
              eventHandlers={{ click: getUserChoice }}
            >
              <Popup>
                {isCorrectAnswer === null ? null : isCorrectAnswer === true ? (
                  <P>{goodAnswer}</P>
                ) : (
                  <P> {badAnswer}</P>
                )}
